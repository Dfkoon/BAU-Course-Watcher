...
يدعم:
1) الربط التلقائي والاشتراك المباشر (One-Click) مع الحماية الكاملة من الأخطاء (Safe Execution).
...
def get_telegram_chat_id(email):
    ...
    try:
        ...
        log.error(f"get_telegram_chat_id error: {e}")
        return None
...
def send_telegram_message(chat_id, text):
    ...
    try:
        ...
        if not resp.ok: 
            log.warning(f"tg send fail {resp.status_code}: {resp.text}")
            # في حال وجود خطأ في تنسيق HTML، حاول الإرسال كنص عادي
...
def _format_times_friendly(times_raw):
    if not times_raw or times_raw == '—': return "غير محدد"
    try:
        DAY_MAP = {'ح': 'الأحد', 'ن': 'الإثنين', 'ث': 'الثلاثاء', 'ر': 'الأربعاء', 'خ': 'الخميس', 'س': 'السبت',
                   '1': 'الأحد', '2': 'الإثنين', '3': 'الثلاثاء', '4': 'الأربعاء', '5': 'الخميس'}
        
        # 1. إذا كان JSON
        if isinstance(times_raw, str) and times_raw.startswith("["):
            arr = json.loads(times_raw)
            if arr:
                formatted = []
                for item in arr:
        ...
        # 2. إذا كان نمط نصي عربي (مثل: ح ث 09:30 10:30)
        pattern = re.compile(r'((?:[حنثرخس]\s*)+)(\d{2}:\d{2})\s+(\d{2}:\d{2})')
        matches = pattern.findall(str(times_raw))
        if matches:
            parts = []
            for days_raw, t1, t2 in matches:
                days_list = [DAY_MAP.get(d, d) for d in days_raw.split() if d in DAY_MAP]
                days_fmt = ' - '.join(days_list)
                parts.append(f'{days_fmt}: {t1} - {t2}')
            return ' | '.join(parts)
    except Exception:
        pass
...
def search_courses_in_db(query):
    ...
    try:
        ...
        log.error(f"search_courses_in_db error: {e}")
        return []
    ...
    try:
        ...
        if text.startswith("/start"):
            ...
            if row:
                ...
                subs = conn.execute("SELECT * FROM subscribers WHERE email=? AND active=1 ORDER BY id DESC LIMIT 10", (row["email"],)).fetchall()
        ...
        if not results:
            not_found_msg = (
                ...
                f"2. أو اكتب جزءاً أساسياً من اسم المادة (مثال: <i>هياكل</i> أو <i>برمجة</i>).\n"
        ...
        log.error(f"telegram_webhook critical error: {e}", exc_info=True)
        return "OK", 200
